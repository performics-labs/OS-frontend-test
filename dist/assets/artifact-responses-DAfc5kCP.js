const t={"button-component":{kind:"code",language:"tsx",title:"Button Component",description:"A reusable button component with variant support",content:`import { ButtonHTMLAttributes } from 'react';
...
}`},"data-table":{kind:"code",language:"tsx",title:"DataTable Component",description:"Generic data table with sorting and filtering",content:`import { useState } from 'react';
...
}`},"markdown-doc":{kind:"text",title:"Component Documentation",description:"Markdown documentation for React components",content:`# React Component Guide
...
\`\`\`
`},"api-hook":{kind:"code",language:"tsx",title:"useAPI Hook",description:"Custom React hook for API calls with loading and error states",content:`import { useState, useEffect } from 'react';
...
`},"python-analysis":{kind:"code",language:"python",title:"Data Analysis Script",description:"Python script for analyzing and visualizing data",content:`import pandas as pd
...
`},"simple-chart-svg":{kind:"image",title:"Bar Chart Visualization",description:"SVG bar chart showing sample data",content:`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300">
...
</svg>`},"sample-image-base64":{kind:"image",title:"Generated Landscape",description:"AI-generated image",content:`<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400" viewBox="0 0 600 400">
  <!-- Sky -->
...
</svg>`},"sales-data-spreadsheet":{kind:"spreadsheet",title:"Q1 Sales Report",description:"Quarterly sales data by region and product",content:`Product,Region,Q1 Sales,Units Sold,Growth %
...
Mouse Wireless,Asia Pacific,18000,2500,32.5`},"customer-data-spreadsheet":{kind:"spreadsheet",title:"Customer Database",description:"Customer information with contact details",content:`Customer ID,Name,Email,Phone,Status,Last Purchase
...
CUST010,Hannah Montana,hannah.m@email.com,555-0110,Active,2024-01-28`}};function r(a){const e=a.toLowerCase();return/(?:create|build|make|generate|write).*button|button.*(?:component|code)/i.test(e)?t["button-component"]:/(?:create|build|make|generate).*(?:table|grid|datatable)|(?:table|grid).*component/i.test(e)?t["data-table"]:/(?:write|create|generate).*(?:guide|docs?|documentation)|(?:component|react).*(?:guide|docs?)/i.test(e)?t["markdown-doc"]:/(?:write|create|build|make).*hook|(?:api|custom).*hook|use[A-Z]/i.test(e)?t["api-hook"]:/(?:write|create|generate).*(?:python|data analysis|visualization)|python.*(?:script|code)/i.test(e)?t["python-analysis"]:/(?:create|generate|make|draw).*(?:chart|graph|svg|visualization)|(?:bar|line|pie).*(?:chart|graph)/i.test(e)?t["simple-chart-svg"]:/(?:create|generate|make|draw).*(?:image|picture|png|photo)/i.test(e)?t["sample-image-base64"]:/(?:create|generate|make|show).*(?:sales|revenue|quarterly|q1|q2|q3|q4).*(?:data|report|spreadsheet)|(?:sales|revenue).*(?:spreadsheet|table|csv)/i.test(e)?t["sales-data-spreadsheet"]:/(?:create|generate|make|show).*(?:customer|client|contact|user).*(?:data|list|spreadsheet)|(?:customer|contact).*(?:spreadsheet|database|table)/i.test(e)?t["customer-data-spreadsheet"]:/(?:create|generate|make).*(?:spreadsheet|csv|excel|table data)|(?:spreadsheet|csv).*(?:file|data)/i.test(e)?t["sales-data-spreadsheet"]:null}export{t as M,r as g};
